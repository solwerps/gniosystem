...
// Orden fijo de grupos y colores “altos”
...
export default function EditarRegimenIvaEmpresaPage() {
  ...
  // Agrupar por Nombre Común con orden fijo y luego alfabético
  ...
  const guardar = async () => {
    try {
      ...
      router.push(`/dashboard/empresa/${usuario}/regimen/iva`);
    } catch (e) {
      console.error(e);
  ...
  // Mezcla de un slice del grupo en el arreglo global y reindexa todo
  const applyGroupSlice = (groupName: string, updatedSlice: RegimenIvaFila[]) => {
    setRows((prev) => {
      ...
      // conservamos el orden relativo: pegamos updatedSlice en el lugar del grupo
      // Para mantener el orden de grupos, reordenamos por índice original:
  ...
  // Proxy de tabla por grupo (reutiliza tu RegimenIvaTable)
