...
// mismos colores que en la pantalla de crear
...
export default function NomenclaturaDetalleContadorPage() {
  ...
  const [expandedNivel2, setExpandedNivel2] = useState<Set<string>>(() => new Set());
  const [expandedNivel3, setExpandedNivel3] = useState<Set<string>>(() => new Set());
  ...
  // Orden estable por orden
  const sortedRows = useMemo(
    () => [...rows].sort((a, b) => a.orden - b.orden),
    [rows]
  );

  // ===== jerarquía por cuenta (padres para niveles 2, 3, 4) =====
  ...
  // ======== REGLAS ACCIONES: "+" y "🗑" ========
  // Mismo criterio que en crear: solo se pueden crear cuentas de nivel 4 y que no sean plantilla / lockAdd
  ...
  const addAfter = (r: CuentaRow) => {
    ...
    setRows((prev) => {
      ...
      const nueva: CuentaRow & { _leadingRoot?: string } = {
        ...
        principalDetalle: "D",
        nivel: 4,
        tipo: (r.tipo ?? "BALANCE GENERAL") as Tipo,
  ...
  // ====== colores de fila (si luego quieres diferenciarlos por grupo/nivel aquí) ======
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {pageRows.map((r) => {
                      ...
                      // Color del botón NIVEL según jerarquía
                      const rootRowForNivel = getRootRow(r);
                      let nivelColorClass = "bg-slate-800";
                      if (rootRowForNivel) {
                        const c = String(rootRowForNivel.cuenta ?? "") as RootId;
                      ...
                      return (
                        <tr
                          key={r.orden}
                          className={cx("border-b", getRowBg(r), rowIsGrey && "bg-slate-100")}
                        >
                          {/* ACCIONES */}
                          ...
                          {/* PRINCIPAL / DETALLE (Desplegar para raíces) */}
                          <td className="px-3 py-2">
                            {isRoot && rootId ? (
                              <button
                                className={cx(
