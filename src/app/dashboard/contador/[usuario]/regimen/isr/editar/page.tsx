//src/app/dashboard/contador/[usuario]/regimen/isr/editar/page.tsx
"use client";
...
const BTN_COLORS: Record<string, string> = {
  "ISR Mensual": "bg-sky-600 hover:bg-sky-700 focus:ring-sky-300",
...
export default function EditarRegimenIsrContadorPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const arr: any[] = (payload?.data ?? payload) || [];
  ...
  const applyGroupSlice = (groupName: string, updated: RegimenIsrFila[]) => {
    setRows((prev) => {
  ...
  const guardar = async () => {
    try {
      ...
      router.push(`/dashboard/contador/${usuario}/regimen/isr`);
    } catch (e) {
      console.error(e);
