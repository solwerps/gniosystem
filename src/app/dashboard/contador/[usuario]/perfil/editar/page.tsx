// src/app/dashboard/contador/[usuario]/perfil/editar/page.tsx
"use client";
...
export default function EditarPerfilContadorPage() {
  ...
  function generatePassword(len = 12) {
    ...
    while (pwd.length < len) pwd.push(all[Math.floor(Math.random() * all.length)]);
    // Mezclar
    ...
    try {
      ...
      const res = await fetch("/api/me", {
        ...
        body: JSON.stringify({
          ...
          // En el backend (PUT /api/me) se hashea password si viene no vacío
        }),
      });
      ...
      // Si cambió username/rol, el backend devuelve redirect correcto
      router.push(j2.redirect || `/dashboard/contador/${usuario}/perfil`);
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="flex">
        <Sidebar role="CONTADOR" usuario={usuario as string} />
      ...
      <main className="flex-1 min-h-screen bg-gray-50 px-8 py-10">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl font-extrabold tracking-tight mb-2">
            Perfil/<span className="font-semibold">Editar</span>
          </h1>
          <a
            href={`/dashboard/contador/${usuario}/perfil`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Por seguridad no se muestra la contraseña actual (se guarda como hash). Aquí puedes establecer una nueva.
              </p>
            </div>
