...
export async function GET(request: Request) {
  try {
    ...
    if (startDate && endDateExclusive) {
      where.fecha = { gte: startDate, lt: endDateExclusive };
    } else if (startDate) {
      where.fecha = { gte: startDate };
    } else if (endDateExclusive) {
      where.fecha = { lt: endDateExclusive };
    }
    ...
    return NextResponse.json(
      {
        status: 200,
        data,
        message: "Asientos de balance obtenidos correctamente.",
      },
      { status: 200 }
    );
  } catch (error) {
    ...
    return NextResponse.json(
      {
        status: 500,
        data: [],
        message: "Error interno al obtener asientos de balance.",
      },
      { status: 500 }
    );
  }
